...
import { bsCappedCall, bsPut } from "./bs.js";
import { config } from "./config.js";

const YEAR_SEC = 365 * 24 * 3600;

const account = privateKeyToAccount(config.quoterPrivateKey);
...
const domain = {
  ...
  verifyingContract: config.coreAddress,
} as const;
...
export async function buildSignedQuote(req: QuoteRequest, spot: number) {
  ...
  const fairPerBtc = req.isPut
    ? bsPut(spot, req.strikeUsd, T, config.iv, config.riskFreeRate)
    : bsCappedCall(spot, req.strikeUsd, req.capUsd!, T, config.iv, config.riskFreeRate);
  const fairUsd = fairPerBtc * req.qtyBtc;
  const bidUsd = fairUsd * (1 - config.spread);
  ...
  const quote = {
    ...
    premium,
    quoteDeadline: BigInt(now + config.quoteTtlSec),
